...
task = tasks.TWO_ORIENTATIONS_DOUBLE_OUTPUT_O1(ori=2)
...
if redo_preanalysis:
    ...
    task = tasks.TWO_ORIENTATIONS_DOUBLE_OUTPUT_O1(orientation_neurons, hold_orientation_for, hold_cue_for, delay0_set, delay1_set, delay2_set,
                                            simple_input=simple_input, simple_output=simple_output, ori=2)
...
def network_performance():
    ...
    MSE_O2 = (error[0]+error[1]).item()
    MSE_O1 = 0#(error[2]+error[3]).item()
    trig = megabatch_output[0][:, t5+1:t6+1, :]
    o2 = torch.atan2((trig[:, :, 0]/(trig[:, :, 0]**2+trig[:, :, 1]**2)**0.5), (trig[:, :, 1]/(trig[:, :, 0]**2+trig[:, :, 1]**2)**0.5)) / 2 * 180 / math.pi
    o1 = 0#torch.atan2((trig[:, :, 2]/(trig[:, :, 2]**2+trig[:, :, 3]**2)**0.5), (trig[:, :, 3]/(trig[:, :, 2]**2+trig[:, :, 3]**2)**0.5)) / 2 * 180 / math.pi
    trig = megabatch_target[:, t5+1:t6+1, :]
    o2_t = torch.atan2((trig[:, :, 0]/(trig[:, :, 0]**2+trig[:, :, 1]**2)**0.5), (trig[:, :, 1]/(trig[:, :, 0]**2+trig[:, :, 1]**2)**0.5)) / 2 * 180 / math.pi
    o1_t = 0#torch.atan2((trig[:, :, 2]/(trig[:, :, 2]**2+trig[:, :, 3]**2)**0.5), (trig[:, :, 3]/(trig[:, :, 2]**2+trig[:, :, 3]**2)**0.5)) / 2 * 180 / math.pi
    error_o2 = torch.minimum(torch.minimum((o2-o2_t)**2, (o2-o2_t+180)**2), (o2-o2_t-180)**2)
    error_o2 = torch.mean(error_o2).item()**0.5
    error_o1 = 0#torch.minimum(torch.minimum((o2-o2_t)**2, (o2-o2_t+180)**2), (o2-o2_t-180)**2)
    error_o1 = 0#torch.mean(error_o2).item()**0.5
