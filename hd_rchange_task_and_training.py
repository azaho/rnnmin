...
class Model(torch.nn.Module):
    def __init__(self):
        ...
        with torch.no_grad():
            ...
            self.fc_h2y.weight[2, R2_i] = torch.sin(2 * R2_pref / 180 * np.pi) * 0.0725
    ...
    def forward(self, input, noise):
        ...
        for j, i in enumerate(R1):
            in_pref_shift = self.r1_pref_shifts[j] # ((j*7)%orientation_neurons - orientation_neurons/2)*4
            in_pref = torch.arange(orientation_neurons) / orientation_neurons * 180 - in_pref_shift
            put_weights("IN", [i], units_from_pref=in_pref, units_to_pref=R1_pref[j:j + 1], m=self.r_r_m)  # 16)
        # put_weights(model, "IN", DT, m=0.16)
        put_weights("IN", R2, m=self.r_r_m)
        put_weights("IN", DT, m=self.in_dt_m, weight_fun=weight_ns)

        put_weights(R1, R1, m=self.r_r_m)
        # put_weights(model, DT, DT, m=0.16*R1_N/DT_N)
        put_weights(R2, R2, m=self.r_r_m)

        put_weights(R1, DT, units_to_pref=DT_pref, m=self.r1_dt_m, weight_fun=weight_ns)
